...
Value REMOTE_AS (\d+)
Value UP_DOWN (\S+)
Value STATE_PFXRCVD (Idle|Connect|Active|OpenSent|OpenConfirm|Established|\d+)

Start
  ^BGP.+identifier\s+${ROUTER_ID}.+AS\s+number\s+${LOCAL_AS}\s*$$
  ^Neighbor.*PfxRcd$$ -> SearchPattern

SearchPattern
#  ^${PEER_IP}\s+\d\s+${REMOTE_AS}\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+${UP_DOWN}\s+${STATE_PFXRCVD}\s*$$ -> Record
  ^${PEER_IP}\s+\d\s+${REMOTE_AS}.+0\s${UP_DOWN}\s+${STATE_PFXRCVD}\s*$$ -> Record
